...
if (!function_exists('newTicketArrivedNotification')) {
	...
	function newTicketArrivedNotification($ticket)
	{
		...
		$mail_data = [
			...
			'_ticket_link_' => 'https://' . env('CENTRAL_DOMAIN') . '/admin/'.getSaasPrefix().'/support-ticket-details/' . $ticket->id,
		...
		foreach ($admins as $admin) {
			...
			$data = [
				...
				'link' => '/admin/'.getSaasPrefix().'/support-ticket-details/' . $ticket->id
			];
	...
	function newReplyArrivedNotification($ticket, $ticket_reply)
	{
		...
		foreach ($all_users as $user) {
			...
			if ($user->user_type == config('saas.user_type.subscriber')) {
				$mail_data['_ticket_link_'] = 'https://' . env('CENTRAL_DOMAIN') . '/user/'.getSaasPrefix().'/support-ticket-details/' . $ticket->id;
			} else {
				$mail_data['_ticket_link_'] = 'https://' . env('CENTRAL_DOMAIN') . '/admin/'.getSaasPrefix().'/support-ticket-details/' . $ticket->id;
			}
			...
			if ($user->user_type == config('saas.user_type.subscriber')) {
				$data['link'] = '/user/'.getSaasPrefix().'/support-ticket-details/' . $ticket->id;
			} else {
				$data['link'] = '/admin/'.getSaasPrefix().'/support-ticket-details/' . $ticket->id;
			}
