                            ...
                            @foreach ($tickets as $ticket)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @endphp
                                        <div>
                                            <span><b>{{ translate('Subject : ') }}</b> {{ $ticket->subject }}</span><br>
                                            <span class="{{ $priority_class }}"><b>{{ translate('Type : ') }}</b>
                                                {{ strtoupper($ticket->priority) }}</span><br>
                                            <span><b>{{ translate('Category : ') }}</b>
                                                {{ $ticket->categoryDetails->name }}</span><br>
                                            <span><b>{{ translate('Assigned To : ') }}</b>
                                                {{ !empty($ticket->assigned_to) ? $ticket->assignedTo->name : '' }}</span><br>
                                            <span><b>{{ translate('Status : ') }}</b>
                                                {{ $status_name }}</span>
                                        </div>
                                    </td>
                                    <td>{{ $ticket->createdBy->name }}</td>
                                    @if (!empty($ticket->lastReply()))
                                        <td>{{ $ticket->lastReply()->repliedBy->name }}</td>
                                    @else
                                        <td>-</td>
                                    @endif
                                    @if (!empty($ticket->lastReply()))
                                    ...
                                    <td class="text-right w-20">
                                        <div class="dropdown-button">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <a
                                                    href="{{ route('edit.'.$route_extension.'support.tickets', $ticket->id) }}">{{ translate('Edit') }}</a>
                                                <a
                                                    href="{{ route($route_extension.'support.ticket.details', $ticket->id) }}">{{ translate('Details') }}</a>
                                                <a href="#"
