...
class CustomDomainController extends Controller
{
    ...
    public function requestCustomDomain(Request $request)
    {
        ...
        try {
            ...
            $package_privillages = DB::table('tl_saas_package_has_privileges')
                ->where('package_id', '=', $domain->saasAccount->package_id)
                ->value('privileges');

            $package_privillages = json_decode($package_privillages);

            $custom_domain_privillage = $package_privillages->package_privileges_custom_domain;


            if ($custom_domain_privillage!= -1 && (int)$total_custom_domain_request >= (int)$custom_domain_privillage) {
                return response()->json([
                    'success' => false,
            ...
            newCustomDomainRequest($domain->saasAccount->id);
            DB::commit();
