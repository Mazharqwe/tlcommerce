...
<!-- Main Content -->
<form class="form-horizontal my-4 mb-4" id="update-package-form" action="{{ route('plugin.saas.update.package') }}" method="post" enctype="multipart/form-data">
    @csrf
        ...
        <div class="col-md-12">
            ...
            <div class="row">
                ...
                <div class="col-12">
                    <ul class="nav nav-tabs nav-fill border-light border-0">
                        ...
                        <li class="nav-item">
                            <a class="nav-link @if ($language->code == $lang) active border-0 @else bg-light @endif py-3" href="{{ route('plugin.saas.edit.package', ['id' => $package->id, 'lang' => $language->code]) }}">
            ...
            <div class="card mb-30">
                <div class="d-sm-flex justify-content-between align-items-center card-header">
                    <h4 class="font-20">{{ translate('Package Details') }}</h4>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="form-group my-4 col-md-4">
                            <label for="name" class="font-14 bold black mb-2">{{ translate('Name') }}<span class="text-danger"> * </span>
                            </label>
                            <input type="text" name="name" id="name" class="theme-input-style" value="{{ $package->translation('name', $lang) }}" placeholder="{{ translate('Name') }}" required>
                            @if ($errors->has('name'))
                            <p class="text-danger">{{ $errors->first('name') }}</p>
                            @endif
                        </div>

                        <div class="form-row my-4 col-md-4 {{ !empty($lang) && $lang != getdefaultlang() ? 'area-disabled' : '' }}">
                        ...
                        <div class="form-row my-4 col-md-4 mb-2 {{ !empty($lang) && $lang != getdefaultlang() ? 'area-disabled' : '' }}">
                    ...
                    <div class="row {{ !empty($lang) && $lang != getdefaultlang() ? 'area-disabled' : '' }}">
                        <div class="form-row my-4 col-md-4 mb-2">
                        ...
                        <div class="form-group my-4 col-md-4 mb-2" id="trail_period_div">
                            <label class="font-14 bold black">{{ translate('Trial Period') }} <span class="text-danger"> *
                                </span></label>
                            <input type="number" name="trail_period" id="trail_period" class="theme-input-style" value="{{ $package->trail_period }}" placeholder="{{ translate('Trail Period') }}" min='0'>
            ...
            <div class="card mb-30 pricing_plan {{ !empty($lang) && $lang != getdefaultlang() ? 'area-disabled' : '' }}">
                <div class="d-sm-flex justify-content-between align-items-center card-header">
                    <h4 class="font-20">{{ translate('Select Pricing Plans') }} <span class="text-danger"> *
                        </span></h4>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="d-flex form-group my-4 col-md-10">
                            <div class="form-group my-4 col-md-10" id="select_plans">
                                <select class="theme-input-style" name="plans[]" multiple="multiple" id="multi_plan_input" onchange="addPlanPrice()">
                                    @for ($i = 0; $i < sizeof($plans); $i++) <option value="{{ $plans[$i]->id }}" data-name="{{ $plans[$i]->name }}" {{ in_array($plans[$i]->id, $selected_planing_id) ? 'selected' : '' }}>
            ...
            <div class="card mb-30 pricing_plan {{ !empty($lang) && $lang != getdefaultlang() ? 'area-disabled' : '' }}">
                <div class="d-sm-flex justify-content-between align-items-center card-header">
                    <h4 class="font-20">{{ translate('Package Pricing') }}</h4>
                </div>
                <div class="card-body">
                    <div class="row" id="pricing_plans">
                        @foreach ($selected_planing as $plan)
                        <div class="form-group my-4 col-md-4">
                            <label for="name_{{ strtolower($plan->name) }}" class="font-14 bold black mb-2">{{ translate($plan->name) }}
                                {{ translate('Cost') }}
                            </label>
                            <input type="number" name="cost[]" id="{{ strtolower($plan->name) }}_cost" class="form-control" value="{{ $plan->cost }}" placeholder="{{ translate($plan->name) }} {{ translate('Cost') }}">
                        </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="row {{ !empty($lang) && $lang != getdefaultlang() ? 'area-disabled' : '' }}">
                <div class="col-md-4">
                    <div class="card mb-30 plan_features">
                        <div class="d-sm-flex justify-content-between align-items-center card-header">
                            <h4 class="font-20">{{ translate('Choose Features') }}</h4>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="form-group d-flex justify-content-between">
                                ...
                                <label class="switch glow primary medium">
                                    @if (in_array($plugin->id, array_column($package_features, 'plugin_id')))
                                    <input type="checkbox" name="package_plugins_{{ $plugin->id }}" value="{{ $plugin->id }}" checked>
                                    @else
                                    <input type="checkbox" name="package_plugins_{{ $plugin->id }}" value="{{ $plugin->id }}">
                                    @endif
                ...
                <div class="col-md-4">
                    <div class="card mb-30 plan_features">
                        <div class="d-sm-flex justify-content-between align-items-center card-header">
                            <h4 class="font-20">{{ translate('Access Privileges') }}</h4>
                        </div>
                        <div class="card-body">
                            @php
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="package_privileges_create_page" class="theme-input-style" value="{{ isset($privileges) ? $privileges['package_privileges_create_page'] : 0 }}" placeholder="{{ translate('Number of pages can create') }}" min="-1" required>
                                <label class="font-12 bold">{{ translate('-1 For Unlimitted') }}</label>
                            </div>
                            <div class="form-group">
                                <label class="font-14 bold black mb-2">{{ translate('Create Blog') }}</label>
                                <input type="number" name="package_privileges_create_blog" class="theme-input-style" value="{{ isset($privileges) ? $privileges['package_privileges_create_blog'] : 0 }}" placeholder="{{ translate('Number of blogs can create') }}" min="-1" required>
                                <label class="font-12 bold">{{ translate('-1 For Unlimitted') }}</label>
                            </div>
                            <div class="form-group">
                                <label class="font-14 bold black mb-2">{{ translate('Store Product') }}</label>
                                <input type="number" name="package_privileges_create_product" class="theme-input-style" value="{{ isset($privileges) ? $privileges['package_privileges_create_product'] : 0 }}" placeholder="{{ translate('Number of products can upload') }}" min="-1" required>
                                <label class="font-12 bold">{{ translate('-1 For Unlimitted') }}</label>
                            </div>
                            <div class="form-group">
                                <label class="font-14 bold black mb-2">{{ translate('Customers') }}</label>
                                <input type="number" name="package_privileges_customers" class="theme-input-style" value="{{ isset($privileges) ? $privileges['package_privileges_customers'] : 0 }}" placeholder="{{ translate('Number of customers can have') }}" min="-1" required>
                                <label class="font-12 bold">{{ translate('-1 For Unlimitted') }}</label>
                            </div>
                            <div class="form-group">
                                <label class="font-14 bold black mb-2">{{ translate('Custom Domain') }}</label>
                                <input type="number" name="package_privileges_custom_domain" class="theme-input-style" value="{{ isset($privileges) ? $privileges['package_privileges_custom_domain'] : 0 }}" placeholder="{{ translate('Number of custom domains') }}" min="-1" required>
                                <label class="font-12 bold">{{ translate('-1 For Unlimitted') }}</label>
                            </div>
                            <div class="form-group">
                                <label class="font-14 bold black mb-2">{{ translate('Maximum Allocated Storage (MB)') }}</label>
                                <input type="number" name="package_privileges_allocated_storage" class="theme-input-style" value="{{ isset($privileges) ? $privileges['package_privileges_allocated_storage'] : 0 }}" placeholder="{{ translate('Maximum Allocated Storage') }}" min="-1" required>
                                <label class="font-12 bold">{{ translate('-1 For Unlimitted') }}</label>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-4">
                    <div class="card mb-30 plan_features">
                        <div class="d-sm-flex justify-content-between align-items-center card-header">
                            <h4 class="font-20">{{ translate('Payment Gateways') }}</h4>
                        </div>
                        <div class="card-body">
                            @foreach ($payment_methods as $method => $id)
                            <div class="form-group d-flex justify-content-between">
                                @if ($method == 'cod')
                                <label class="font-14 bold black mb-2">{{ translate('Cash On Delivery') }}</label>
                                @else
                                <label class="font-14 bold black mb-2">{{ ucfirst($method) }}</label>
                                @endif

                                <label class="switch glow primary medium">
                                    @if (in_array($id, array_column($package_payment_methods, 'payment_method')))
                                    <input type="checkbox" name="package_payment_method_{{ $id }}" value="{{ $id }}" checked>
                                    @else
                                    <input type="checkbox" name="package_payment_method_{{ $id }}" value="{{ $id }}">
                                    @endif
    ...
    function addPlanPrice(event) {
        ...
        for (let i = 0; i < selectedOptionsValue.length; i++) {
            ...
            if (selected_index_in_planings != -1) {
                ...
                html = html + `<div class="form-group my-4 col-md-4">
