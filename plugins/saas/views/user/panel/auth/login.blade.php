...
$login_bg_image = !empty($auth_layout['user_login_bg_image']) ? asset(getFilePath($auth_layout['user_login_bg_image'])) : '';
@endphp
                ...
                <div class="card-body">
                    <form action="{{ route('subscriber.attemptLogin') }}" method="post" id="login-form">
                        ...
                        <div class="d-flex align-items-center mb-20 mt-2">
                            <button type="submit" class="btn long w-100" id="login">
                                {{ translate('Log In') }}
                            </button>
                        </div>

                        <div class="d-flex justify-content-center">
                            <a href="{{ route('subscriber.password.reset.link') }}" class="font-12 text_color">{{ translate('Forgot Password?') }}</a>
                        </div>
                        <div class="d-flex justify-content-center">
                            <span class="font-12 text_color">{{ translate('Not registered yet? ') }}</span>
                            <a href="{{ route('plugin.saas.user.registration') }}" class="font-12 text_color ml-1">{{ translate('Register Now') }}</a>
                        </div>
                    </form>
                </div>
            </div>
