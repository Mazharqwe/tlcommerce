...
<form method="post" class="form-horizontal mb-4" autocomplete="off" action="{{ route('plugin.saas.update.coupon') }}" enctype="multipart/form-data">
    @csrf
            ...
            <div class="card mb-30">
                <div class="card-body row">
                    <div class="d-sm-flex justify-content-between align-items-center m-3">
                        <h4 class="font-20">{{ translate('Update Coupon') }}</h4>
                    </div>
                    ...
                    <div class="my-3 col-md-12">
                        <label class="font-14 bold black mb-2">{{ translate('Coupon Name') }} <span class="required">
                                *</span></label>
                        <input type="text" class="theme-input-style" name="coupon_name" id="coupon_name" value="{{ $coupon->coupon_name }}" readonly>
                    ...
                    <div class="my-3 col-md-12">
                        ...
                        <label class="font-14 bold black mb-2">{{ translate('Coupon Type') }} <span class="required">
                                *</span></label>
                        <select class="theme-input-style" name="coupon_type" id="coupon_type" onchange="togglePackageSelectionInputs()" disabled>
                    ...
                    <div class="my-3 col-md-12" id="single_packages">
                        <label class="font-14 bold black mb-2">{{ translate('Applicable Package') }} <span class="required">
                                *</span></label>
                        <select class="theme-input-style" name="package">
                            @for ($i = 0; $i < sizeof($packages); $i++) <option value="{{ $packages[$i]->id }}" {{ collect(getAllPackageIdsByCouponId($coupon->id))->contains($packages[$i]->id) ? 'selected' : '' }}>
                                {{ $packages[$i]->name }}
                                </option>
                                @endfor
                        </select>
                    ...
                    <div class="my-3 col-md-12" id="multi_packages">
                        <label class="font-14 bold black mb-2">{{ translate('Applicable Packages') }} <span class="required">
                                *</span></label>
                        <select class="theme-input-style" name="packages[]" multiple="multiple" id="multi_package_input">
                            @for ($i = 0; $i < sizeof($packages); $i++) <option value="{{ $packages[$i]->id }}" {{ collect(getAllPackageIdsByCouponId($coupon->id))->contains($packages[$i]->id) ? 'selected' : '' }}>
                                {{ $packages[$i]->name }}
                                </option>
                                @endfor
                    ...
                    <div class="my-3 col-md-12">
                        <label class="font-14 bold black mb-2">{{ translate('Number Of Times This Coupon Can Be Used') }}
                            <span class="required">
                                *</span></label>
                        <input type="number" min="1" class="theme-input-style" name="coupon_usable_times" id="coupon_usable_times" value="{{ $coupon->coupon_usable_times }}">
                    ...
                    <div class="my-3 col-md-12">
                        <label class="font-14 bold black mb-2">{{ translate('Coupon Code') }} <span class="required">
                                *</span></label>
                        <input type="text" class="theme-input-style" name="coupon_code" id="coupon_code" value="{{ $coupon->coupon_code }}" readonly>
                    ...
                    <div class="my-3 col-md-12" id="discount">
                        <label class="font-14 bold black mb-2">{{ translate('Discount(%)') }} <span class="required">
                                *</span></label>
                        <input type="number" class="theme-input-style" name="discount" min="0" value="{{ $coupon->discount }}">
                    ...
                    <div class="my-3 col-md-12">
                        <label class="font-14 bold black mb-2">{{ translate('Days(Valid For)') }} <span class="required">
                                *</span></label>
                        <input type="number" class="theme-input-style" name="valid_days" min="1" value="{{ $coupon->valid_for_days }}">
                    ...
                    <div class="my-3 col-md-12">
                        <button class="btn btn-success sm" onclick="save(event)">{{ translate('Update') }}</button>
                    </div>
                </div>
            </div>
        </div>
