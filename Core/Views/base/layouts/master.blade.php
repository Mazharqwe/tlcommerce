        ...
        <div class="main-wrapper">
            ...
            <div class="main-content">
                <div class="container-fluid">
                    ...
                    $upgrade_url = env('APP_URL') . '/user/'.getSaasPrefix().'/change-subscription-plan/' . $saas_account_id;
                    $user_type = auth()->user()->user_type;
                    @endphp
                    @if (auth()->check() &&
                    $user_type == config('tlecommercecore.user_type.admin') &&
                    !empty($crossed_privileges) && isTenant())
                    <div class="alert alert-warning align-items-center d-flex justify-content-between" role="alert">
                        ...
                        <a href="{{ $upgrade_url }}" class="btn btn-danger sm btn-square float-right p-2">{{ translate('Upgrade Subscription') }}</a>
                    </div>
                    @endif
                    @if (auth()->check() &&
                    $user_type == config('tlecommercecore.user_type.seller') &&
                    session()->get('product_limit_over') == 1 && isTenant())
                    <div class="alert alert-warning align-items-center d-flex justify-content-between" role="alert">
