    ...
    function initDropzone() {
        ...
        $("#uploaded").dropzone({
            ...
            accept: function(file, done) {
                if (total_files <= 9) {
                    if (file.type != "image/jpeg" && file.type != "image/svg+xml" && file.type !=
                        "image/jpg" &&
                        file.type != "image/png" &&
                        file
                ...
                for (let i = 0; i < all_files.length; i++) {
                    for (let j = 0; j < selected_file_id.length; j++) {
                        if (all_files[i].id == selected_file_id[j]) {
                            if (all_files[i].path.startsWith("storage")) {
                                let path = all_files[i].path
                ...
                if (is_for_browse_file) {
                    ...
                    for (let i = 0; i < media_files.length; i++) {
                        if (media_files[i].id == id) {
                            if (media_files[i].path.startsWith("storage")) {
                            ...
                            if ($('#list_item_' + id).hasClass('selected')) {
                                ...
                                if (media_files[i].path.startsWith("storage")) {
                                    let path = media_files[i].path
                                    selected_file_location['file_' + id] = path
                                } else {
                                    let path = media_files[i].path
                                    selected_file_location['file_' + id] = path
                                }
                    ...
                    for (let i = 0; i < media_files.length; i++) {
                        if (media_files[i].id == id) {
                            ...
                            if (media_files[i].path.startsWith("storage")) {
                                let media_path = media_files[i].path
                                $('#file_url').val(media_path)
                                $('#download_file').attr('href', media_path)
                            } else {
                                let media_path = media_files[i].path;
                                $('#file_url').val(media_path)
                                $('#download_file').attr('href', media_path)
                            ...
                            if (media_files[i].path.startsWith("storage")) {
                                let media_path = media_files[i].path;
                                $('#preview_image').attr('src', media_path)
                            } else {
                                let media_path = media_files[i].path;
                                $('#preview_image').attr('src', media_path)
                            }
